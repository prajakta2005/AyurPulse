// NewPatient.jsx - Complete working version
"use client"
...
export default function CreateDietChart() {
  ...
  const MEAL_ORDER = ['earlyMorning', 'breakfast', 'midMorning', 'lunch', 'eveningSnack', 'dinner', 'beforeBed'];
  ...
  const handlePredictDosha = async () => {
    const payload = {
      ...
      "Hair Color": "Black", "Appearance of Hair": "Straight",
      "Shape of face": formData.faceShape || "Oval",
      "Eyes": formData.eyeType || "Medium",
      "Eyelashes": "Normal", "Blinking of Eyes": "Normal",
      "Cheeks": "Normal", "Nose": "Normal",
      "Teeth and gums": "Normal", "Lips": "Normal", "Nails": "Normal",
      "Appetite": formData.appetite || "Normal",
      "Liking tastes": "Sweet", "Metabolism Type": "Normal",
  ...
  const handleGenerateDietChart = async () => {
    ...
    try {
      ...
      const result = await response.json();
      if (!result.dietChart) throw new Error("Invalid response format");
  ...
  const generateFallbackDietChart = () => {
    ...
    setDietChart({
      weeklyPlan: Array.from({ length: 7 }, (_, i) => ({
        ...
        meals: {
          earlyMorning: { time: "6:00 AM", items: ["Warm lemon water"], calories: 50, ayurvedicBenefit: "Activates Agni", description: "Start fresh" },
          breakfast: { time: "8:00 AM", items: ["Oats porridge", "Dates"], calories: 400, ayurvedicBenefit: `Balances ${dominantDosha}`, description: "Nourishing breakfast" },
          midMorning: { time: "11:00 AM", items: ["Fresh fruit"], calories: 150, ayurvedicBenefit: "Energy boost", description: "Light snack" },
          lunch: { time: "1:00 PM", items: ["Dal", "Rice", "Vegetables"], calories: 600, ayurvedicBenefit: "Peak digestion", description: "Main meal" },
          eveningSnack: { time: "5:00 PM", items: ["Herbal tea", "Roasted chickpeas"], calories: 200, ayurvedicBenefit: "Prevents overeating", description: "Light protein" },
          dinner: { time: "7:30 PM", items: ["Khichdi", "Raita"], calories: 400, ayurvedicBenefit: "Easy digestion", description: "Light dinner" },
          beforeBed: { time: "9:30 PM", items: ["Turmeric milk"], calories: 100, ayurvedicBenefit: "Deep sleep", description: "Night drink" }
        }
      })),
      doshaBalancingTips: [`Eat warm foods for ${dominantDosha}`, "Regular meal times", "Avoid cold drinks"],
      lifestyleRecommendations: ["Wake before sunrise", "Walk after meals"],
      ayurvedicSupplements: [{ name: "Triphala", benefit: "Digestion", timing: "Before bed" }],
      importantReminders: ["Eat mindfully", "Chew thoroughly"]
    });
    setShowDietChart(true);
  };
  ...
  const handleSubmit = async () => {
    try {
      ...
      alert("Success! Diet chart saved.");
    } catch (error) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-green-50 via-white to-orange-50 py-8 px-4">
        <div className="max-w-7xl mx-auto">
          {/* Header */}
          <div className="text-center mb-8 bg-white rounded-2xl shadow-xl p-8 border-t-4 border-green-600">
            <h1 className="text-5xl font-bold text-green-800 mb-3">
              🌿 Your 7-Day Ayurvedic Diet Chart
            </h1>
            <div className="flex justify-center items-center gap-6 text-lg mt-4">
              <div className="bg-green-100 px-6 py-2 rounded-full">
                <span className="font-semibold text-green-800">Dosha:</span>{' '}
                <span className="text-green-900 font-bold">{formData.dominantDosha || "VATA"}</span>
              </div>
              <div className="bg-orange-100 px-6 py-2 rounded-full">
                <span className="font-semibold text-orange-800">Diet:</span>{' '}
                <span className="text-orange-900 font-bold capitalize">{formData.dietType || "Vegetarian"}</span>
              </div>
            </div>
          </div>
              ...
              <table className="w-full border-collapse">
                <thead>
                  <tr className="bg-gradient-to-r from-green-600 to-green-700">
                    <th className="px-4 py-4 text-left text-white font-bold text-sm uppercase border-r border-green-500">Day</th>
                    {MEAL_ORDER.map(m => (
                      <th key={m} className="px-4 py-4 text-center text-white font-bold text-xs uppercase border-r border-green-500">
                        {mealNames[m]}
                      </th>
                    ))}
                    <th className="px-4 py-4 text-center text-white font-bold text-xs uppercase">Total Cal</th>
                  </tr>
                </thead>
                <tbody>
                  {dietChart.weeklyPlan.map((day, idx) => (
                    <tr key={day.day} className={`border-b hover:bg-green-50 ${idx % 2 === 0 ? 'bg-gray-50' : 'bg-white'}`}>
                      <td className="px-4 py-6 border-r bg-green-50">
                        <div className="font-bold text-green-800 text-lg">{day.dayName}</div>
                        <div className="text-xs text-gray-600 mt-1">Day {day.day}</div>
                        <div className="text-xs text-blue-600 mt-2 font-semibold">💧 {day.waterIntake}</div>
                      </td>
                      {MEAL_ORDER.map(mk => {
                        const meal = day.meals[mk];
                        if (!meal) return <td key={mk} className="px-3 py-4 border-r">-</td>;
                        return (
                          <td key={mk} className="px-3 py-4 border-r align-top">
                            <div className="text-xs text-gray-500 font-semibold mb-1">{meal.time}</div>
                            <div className="space-y-1">
                              {meal.items.map((item, i) => (
                                <div key={i} className="text-xs bg-green-100 text-green-800 px-2 py-1 rounded font-medium">
                                  {item}
                                </div>
                              ))}
                            </div>
                            <div className="text-xs text-orange-600 font-bold mt-2">{meal.calories} kcal</div>
                          </td>
                        );
                      })}
                      <td className="px-4 py-6 text-center bg-orange-50">
                        <div className="text-2xl font-bold text-orange-600">{day.totalCalories}</div>
          ...
          {dietChart.doshaBalancingTips && (
            <div className="bg-gradient-to-br from-green-100 to-green-50 rounded-2xl p-8 mb-6 border-l-4 border-green-600">
              <h3 className="text-2xl font-bold text-green-800 mb-4">🧘 Dosha Balancing Tips</h3>
              <div className="grid md:grid-cols-2 gap-3">
                {dietChart.doshaBalancingTips.map((tip, i) => (
          ...
          <div className="grid md:grid-cols-2 gap-6 mb-8">
            {dietChart.lifestyleRecommendations && (
              <div className="bg-gradient-to-br from-orange-100 to-orange-50 rounded-2xl p-8 border-l-4 border-orange-600">
                <h3 className="text-2xl font-bold text-orange-800 mb-4">🌟 Lifestyle Tips</h3>
                <ul className="space-y-2">
                  {dietChart.lifestyleRecommendations.map((rec, i) => (
                    <li key={i} className="bg-white p-3 rounded-lg shadow-sm flex items-start gap-2">
            ...
            {dietChart.ayurvedicSupplements && dietChart.ayurvedicSupplements.length > 0 && (
              <div className="bg-gradient-to-br from-purple-100 to-purple-50 rounded-2xl p-8 border-l-4 border-purple-600">
                <h3 className="text-2xl font-bold text-purple-800 mb-4">💊 Supplements</h3>
                <div className="space-y-3">
                  {dietChart.ayurvedicSupplements.map((supp, i) => (
                    <div key={i} className="bg-white p-4 rounded-lg shadow-sm">
                      <h4 className="font-bold text-purple-700">{supp.name}</h4>
                      <p className="text-sm text-gray-700 mt-1">{supp.benefit}</p>
                      <p className="text-xs text-gray-600 mt-1">⏰ {supp.timing}</p>
                    </div>
                  ))}
                </div>
              </div>
          ...
          {dietChart.importantReminders && (
            <div className="bg-gradient-to-br from-red-100 to-red-50 rounded-2xl p-8 mb-8 border-l-4 border-red-600">
              <h3 className="text-2xl font-bold text-red-800 mb-4">⚠️ Important Reminders</h3>
              <div className="grid md:grid-cols-2 gap-3">
                {dietChart.importantReminders.map((rem, i) => (
                  <div key={i} className="bg-white p-3 rounded-lg shadow-sm flex items-start gap-2">
                    <span className="text-red-600 font-bold">!</span>
                    <span className="text-gray-700">{rem}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="flex gap-4 justify-center">
            <button onClick={() => setShowDietChart(false)} className="px-10 py-4 bg-gray-600 text-white rounded-full font-bold shadow-xl hover:bg-gray-700 transition-all transform hover:scale-105">
              ← Back
            </button>
            <button onClick={handleSubmit} className="px-10 py-4 bg-green-600 text-white rounded-full font-bold shadow-xl hover:bg-green-700 transition-all transform hover:scale-105">
              💾 Save
            </button>
            <button onClick={() => window.print()} className="px-10 py-4 bg-blue-600 text-white rounded-full font-bold shadow-xl hover:bg-blue-700 transition-all transform hover:scale-105">
              🖨️ Print
            </button>
          </div>
        </div>
              ...
              <div className="flex justify-between mt-8 pt-6 border-t border-gray-200">
                ...
                {currentStep === 3 ? (
                  ...
                  <button
                    ...
                    disabled={isGenerating}
                    className={`px-6 py-2 rounded-md font-medium ${isGenerating
